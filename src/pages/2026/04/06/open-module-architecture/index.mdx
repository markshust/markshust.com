---
title: "Open Module Architecture"
...
I've started calling this setup the **Open Module Architecture**.

![Open Module Architecture](./open-module-architecture.png)

Open Module Architecture is defined by four properties that are held together simultaneously:
...
The word "open" isn't about open source (though Marko is MIT licensed). It's about the architectural property: every module is open to extension by every other module, and doesn’t require an opt-in or the original package author to anticipate your needs. It’s done by default.

The premise comes from the Open/Closed Principle, which is code that is "open for extension, but closed for modification”, but it's elevated from the class design to system architecture. This allows you to extend behavior without modifying source code -- and across module boundaries -- with the framework enforcing the rules.
...
I want to be honest about this: I didn't design Open Module Architecture from the top-down. I didn't read a paper and implement it. I had pain points, and I solved them one at a time.
